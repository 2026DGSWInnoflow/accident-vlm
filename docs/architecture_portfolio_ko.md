...
이 문서는 `accident-vlm` 프로젝트를 포트폴리오에서 설명하기 위한 한글 아키텍처 문서입니다. 핵심 목표는 블랙박스 사고 영상에서 법적 판단이 아닌 객관 사실과 근거를 추출하고, 이후 RAG나 보험/법률 검토 시스템이 사용할 수 있는 구조화 JSON을 만드는 것입니다.
